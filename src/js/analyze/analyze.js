...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.message === "OpenAIResponseCompleted" ) {
        ...
        analyzeButton.className = analyzeButton.className
                                                .replace('bg-purple-500', 'bg-red-700')
                                                .replace('hover:bg-purple-700', 'hover:bg-red-700');
        sleep(2000).then(() => {
            analyzeButton.textContent = 'Analyze Story';
            analyzeButton.className = analyzeButton.className
                                                    .replace('bg-red-700', 'bg-purple-500')
                                                    .replace('hover:bg-red-700', 'hover:bg-purple-700')
        })
    }
});
